export interface MealLibraryItem {
  ...
  portion: string;
  unit: 'serving' | 'g' | 'ml' | 'plate' | 'piece' | 'bowl' | 'pieces' | 'packet' | 'spoon';
...
// Keep the old library for backward compatibility
export const MEAL_LIBRARY: MealLibraryItem[] = INDIAN_MEAL_LIBRARY;

export const getMealsByRegion = (region: string): MealLibraryItem[] => {
  if (region === 'All India') {
    return INDIAN_MEAL_LIBRARY;
  }
  return INDIAN_MEAL_LIBRARY.filter(meal => meal.region === region);
};

export const getMealsByCategory = (category: 'breakfast' | 'lunch' | 'dinner' | 'snack'): MealLibraryItem[] => {
  return INDIAN_MEAL_LIBRARY.filter(meal => meal.category === category);
};

export const getMealsByRegionAndCategory = (region: string, category: 'breakfast' | 'lunch' | 'dinner' | 'snack'): MealLibraryItem[] => {
  let meals = getMealsByCategory(category);
  if (region !== 'All India') {
    meals = meals.filter(meal => meal.region === region);
  }
  return meals;
};
...
export const getBMISuggestedMeals = (bmi: number): MealLibraryItem[] => {
  ...
  return INDIAN_MEAL_LIBRARY.filter(meal => meal.bmiCategory === category);
};